...
function parseHidResponse(event) {
    ...
    if(view[0] == 0x02 && view[1]==0x8a) {
        nrf52_common.device.sendReport(0x00, new Uint8Array([0x02,0x89]));
    }
...
function disableDebug() {
    device.sendReport(0x00, new Uint8Array([0x02, 0x88, 0x00]));
}
...
window.addEventListener("load",()=>{
    ...
    document.getElementById("enable-debug").addEventListener("click", ()=>{
        device.sendReport(0x00,new Uint8Array([0x02, 0x88, 0x01]));
    });
